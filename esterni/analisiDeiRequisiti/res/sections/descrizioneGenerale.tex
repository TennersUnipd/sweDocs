...
\begin{itemize}
	\item creare ed eseguire il deploy di una funzione su \textit{Etherless};
	\item eliminare una funzione da lui pubblicata;
	\item modificare costo, descrizione, parametri, tipo di ritorno della funzione.
\end{itemize}
...
L'intero sistema si basa su tre applicativi software:
	\begin{itemize}
		\item \textbf{\textit{Etherless-cli:}} \textit{CLI\glo} attraverso la quale gli utenti si interfacciano con il servizio ed eseguono le funzionalità disponibili;
		\item \textbf{\textit{Etherless-smart:}} applicativo che si occupa della definizione e del \textit{deploy\glo} dello \textit{smart-contract} sulla rete \textit{Ethereum\glos};
		\item \textbf{\textit{Etherless-server:}} applicativo server che rimane in ascolto e soddisfa le richieste di esecuzione delle funzioni caricate, interfacciandosi con il servizio AWS Lambda mediante l'apposito \textit{SDK\glos}.
	\end{itemize}
