...
\begin{enumerate}
	\item Download the installation package from \\\\\centerline{\code{ https://github.com/TennersUnipd/etherless-cli/releases}}\\
	\item Open a terminal and move the working directory to the enclosing folder of the downloaded .tgz file\\
	\item Install the package through the npm package manager \\\\\centerline{\code{npm install etherless-cli.tgz -g}}\\\\Using the -g option will install the command \code{etherless} globally on your machine
	\item Run the command \code{man etherless} to make sure etherless-cli is correclty installed.
\end{enumerate}
\subsubsection{Usage}
